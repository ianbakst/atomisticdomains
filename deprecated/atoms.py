...
class vasp:
    ...
    def __init__(
        ...
        chem=np.zeros(shape=(1, 1), dtype=str),
        num_atoms=np.zeros(shape=(1, 1)),
        coord=np.zeros(shape=(1, 1)),
        mag_mom=np.zeros(shape=(1, 1)),
        dyn=np.zeros(shape=(1, 1)),
    ):
    ...
    def readposcar(self, filename):
        ...
        self.num_atoms = np.genfromtxt(
            filename, delimiter="", skip_header=6, max_rows=1, dtype=int
        )
        ...
        if ptype == "Selective Dynamics":
            self.coord = np.genfromtxt(
                filename, delimiter="", skip_header=9, max_rows=self.tot_atoms
            )[:, 0:3]
            self.dyn = np.genfromtxt(
                filename, delimiter="", skip_header=9, max_rows=self.tot_atoms, dtype=str
            )[:, 3:]
        else:
            self.coord = np.genfromtxt(
                filename, delimiter="", skip_header=8, max_rows=self.tot_atoms
            )
...
def disl_maker_a(
    ...
    for i in range(s[0]):
        ...
        for j in range(3):
            ...
            U[i, :] = (
                U[i, :] + ln * 0.5 * np.dot(b, ll[:, j]) / np.dot(ll[:, j], aa[:, j]) * aa[:, j]
            )
        for j in range(3, 6):
            ln = np.log(X + S[j] * Y)  # *np.dot(b,LL[:,j])
            U[i, :] = (
...
def disl_maker_i(A, b, l, oo, x, z, nu):
    ...
    for i in range(s[0]):
        ...
        D[i, 1] = B[i, 1] - be / (8 * pi) / (1 - nu) * (
            (1 - 2 * nu) * log(rad) + (B[i, 0] ** 2 - B[i, 1] ** 2) / rad**2
        )
...
def domain_construct(basis_atoms, lx, ly, lz, rx, ry, x, zp):
    ...
    aa = np.array(
        [[0, 0, 0], [0.5 * a, 0.5 * b, 0], [0.5 * a, b / 6, 0.5 * c], [0, 2 * b / 3, 0.5 * c]]
    )
    n = 0
    ts = time.time()
    for i in range(xstart, xend):
        for j in range(ystart, yend):
            for k in range(zstart, zend):
                ...
                tc = time.time() - ts
                print(
                    "Initial lattice %8.2f%% complete. Elapsed time: %s seconds.\r" % (per, tc),
                )
    ...
    for i in range(int(N)):
        ...
        tf = time.time() - ts
        print(
            "Flagging atoms to be cut %8.2f%% complete. Elapsed time: %s seconds.\r" % (per, tf),
        )
    ...
    for j in range(sf[0]):  # effect of force j
        for i in range(s[0]):  # on atom i
            ...
            if per > 0:
                tr = tc / per * (100 - per)
                print(
                    "%8.2f%% complete. Elapsed time: %s seconds. Estimated time remaining: %s seconds.\r"
                    % (per, tc, tr),
                )
            else:
                print(
                    "%8.2f%% complete. Elapsed time: %s seconds. Estimated time remaining: CALCULATING\r"
                    % (per, tc),
                )
    print("\n")
    return u
...
def hcptwin(a, lx, ly, lz, c=sqrt(8 / 3), ex=0, ey=0, pz=0, p=3, type="extension"):
    ...
    twin_plane_box = np.array(
        [(sim_box[0, 1] - sim_box[0, 0]), (sim_box[1, 1] - sim_box[1, 0]), 1.0]
    )
    ...
    if p >= 4:
        top_twin_plane_atoms = add_atoms(
            basis_atoms, unit_cell, twin_plane_box, o=np.array([0.0, 0.0, lz])
        )
...
def remap(A, B, box, p=7, idA=0, idB=0):
    ...
    if sA[0] == sB[0]:
        if sA[1] == 4:
            print(
                "ID info for initial configuration taken from atom list. Additional inputs ignored."
            )
            del idA
            idA = A[:, 0]
            Ia = np.argsort(idA)
            ...
            if sB[1] == 4:
                print(
                    "ID info for final configuration taken from atom list. Additional inputs ignored."
                )
                del idB
                idB = B[:, 0]
                Ib = np.argsort(idB)
                ...
                if len(idB) == sB[1]:
                    ...
                    print(
                        "WARNING: No ID info for final configuration found. Order consistency assumed."
                    )
                    Bpos = B[Ia, :]
            else:
            ...
            if len(idA) == sA[0]:
                ...
                if sB[1] == 4:
                    ...
                    print(
                        "FATAL ERROR: Insufficient number of coordinates for final configuration."
                    )
