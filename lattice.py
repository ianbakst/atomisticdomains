...
class lattice:
	...
	def genfromposcar(self,filename):
		...
		self.box = box
		self.chem = np.genfromtxt(filename, delimiter='', skip_header=5,
								  max_rows=1, dtype=str)
		self.num_el = len(self.chem)
		atom_numbers = np.genfromtxt(filename, delimiter='', skip_header=6,
									 max_rows=1, dtype=int)
		for i in range(self.num_el):
			self.num_atoms[self.chem[i]]=atom_numbers[i]
		self.tot_atoms = sum(atom_numbers)
		ptype = np.genfromtxt(filename, delimiter='>', skip_header=7,
							  max_rows=1, dtype=str)
		if ptype == 'Selective Dynamics':
			...
			for i in range(self.tot_atoms):
				if j < self.num_atoms[self.chem[eln]]:
					...
					atomel = self.chem[eln]
					j = 1
				acoord = np.genfromtxt(filename, delimiter='', skip_header=9+i,
									   max_rows=1)[:, 0:3]
				adyn = np.genfromtxt(filename, delimiter='', skip_header=9+i,
				 					 max_rows=1, dtype=str)[:, 3:]
			...
			for i in range(self.tot_atoms):
				if j < self.num_atoms[self.chem[eln]]:
					...
					atomel = self.chem[eln]
					j = 1
				acoord = np.genfromtxt(filename, delimiter='', skip_header=8+i,
									   max_rows=1)
		...
		if self.mag_mom.shape[0] != self.tot_atoms:
			print ('WARNING: Insufficient number of magnetic moments. \
					Magnetism ignored.')
	...
	def repeated_length(self,direction=np.zeros(shape=(1,3))):
		...
		for i in range(3):
			if direction[i] == 0:
				...
				lengths[i] = 2*sqrt((t * direction[0])**2 + (t * direction[1])**2
							 + (t * direction[2])**2)
		rl = min(lengths)
		return(rl)
